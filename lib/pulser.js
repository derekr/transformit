...
function Pulser (url, options) {
    ...
    this.uploading = true; // inits w/ uploading
}
...
Pulser.prototype.checkStatus = function (assemblyUrl, callback) {
    ...
    if (!this.uploading) return callback(null);

    setTimeout(function () {
        me.fetchAssembly(assemblyUrl, function (err, assembly) {
            if (err) return callback(err);
            
            me.uploading = me.parseAssembly(assembly); // change upload status
            me.checkStatus(assemblyUrl, callback);
        });
    }, this.interval);
};

/**
 * Fetches transloadit assembly for the given assembly url.
 */
Pulser.prototype.fetchAssembly = function (url, callback) {
    $.jsonp({
        url: url + '?seq=' + this.seq, 
...
Pulser.prototype.parseAssembly = function (assembly) {
    ...
    if (assembly.error || (assembly.ok !== 'ASSEMBLY_EXECUTING' && assembly.ok !== 'ASSEMBLY_UPLOADING')) {
        if (assembly.error === 'ASSEMBLY_NOT_FOUND' && this.notFoundAttempts > 0) {
            this.notFoundAttempts--;
