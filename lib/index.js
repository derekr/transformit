...
Transformit.prototype.checkStatus = function (assemblyUrl) {
    ...
    jsonp(assemblyUrl + '?seq=' + this.seq, { 'timeout': this.timeout }, pollHandler);

    function pollHandler (err, assembly) {
        ...
        if (assembly.error || (assembly.ok != 'ASSEMBLY_EXECUTING' && assembly.ok != 'ASSEMBLY_UPLOADING')) {
            if (assembly.error === 'ASSEMBLY_NOT_FOUND') {
                ...
                return me.emit('error', 'Failed to check assembly (' + assembly.message + ')');
            }
        }
...
Transformit.prototype.pollHandler = function () {
